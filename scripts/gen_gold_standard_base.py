"""
Generate haiku standalone outputs for calibration samples (001-005).
These are the base posts Taya will edit to produce gold_standard.jsonl.

Generates blog + linkedin for each sample using the frozen haiku prompts.
Output: data/calibration/gold_standard_base.jsonl

Usage:
    python3 scripts/gen_gold_standard_base.py
"""
...
OUTPUT_FILE  = ROOT / "data" / "calibration" / "gold_standard_base.jsonl"
...
def load_prompt(mode: str) -> tuple[str, str]:
    system = (PROMPTS_DIR / f"haiku_{mode}_system.txt").read_text().strip()
    user   = (PROMPTS_DIR / f"haiku_{mode}_user.txt").read_text().strip()
    return system, user
...
def main():
    ...
    for sample_id in CALIBRATION_IDS:
        ...
        for mode, cfg in MODES.items():
            ...
            system, user_tmpl = load_prompt(mode)
            ...
            try:
                result = call_haiku(system, user_prompt, cfg["max_tokens"])
                record = {
                    "id":                 sample_id,
                    "mode":               mode,
                    ...
                    "baseline_type":      "gold_standard_base",
                }
